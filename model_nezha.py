...
class BertEmbeddings(nn.Module):
    ...
    def forward(self,
        ...
        # ######
        # # position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)  # input_ids没有
...
class NeZhaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.relative_positions_embeddings = _generate_relative_positions_embeddings(
            length=512, depth=self.attention_head_size,
            max_relative_position=config.max_relative_position)  # .cuda()  # TODO
        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)
...
class NezhaPretrain(BertPreTrainedModel):
    def __init__(self, config):
        super(NezhaPretrain, self).__init__(config)
        self.bert = BertModel(config)
        self.im_to_embedding = nn.Linear(2048, 768)  # 图片编码->768

        # padding_idx 按 bert-embedding 填 0
        self.text_embedding = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)

        self.task = BertPreTrainingHeads(config, self.text_embedding.weight)  #
        self.apply(self.init_bert_weights)

    def forward(self,
                input_ids,  # 文本
                token_type_ids,
                attention_mask,  # 文本mask
                labels,  # 文本标签
                unmasked_patch_features,  # 图片
                visual_mask,
                visual_token_type_id,
                is_paired,  # 图文匹配标签
                device):
        embeds = self.text_embedding(input_ids)

        # 图片 文本拼接
        image_embed = self.im_to_embedding(unmasked_patch_features)
        fuse_embed = torch.cat((embeds, image_embed), 1)

        # attention_mask拼接
        attention_mask = torch.cat((attention_mask, visual_mask), 1)
        # TODO: 这儿token_type_id拼接起来做nn.Embedding，
        #  可能需要修成单独做nn.embedding
        #  (参考visualbertembeddings)
        token_id = torch.cat((token_type_ids, visual_token_type_id + 1), 1)

        sequence_output, pooled_output = self.bert(inputs_embeds=fuse_embed,  # bert里面会自己加上位置编码和token编码
                                                   token_type_ids=token_id,
                                                   attention_mask=attention_mask,
                                                   )

        # 将output拆开为text输出和image输出
        text_output = sequence_output[:, :-1, :]
        # hidden states corresponding to the image part
        image_output = sequence_output[:, -1, :]  # 原文用于重建，应该不会用到

        # Predict the masked text tokens and alignment scores (whether image, text match)
        prediction_scores, alignment_scores = self.task(text_output, pooled_output)

        # 只计算对齐的损失
        pred_scores_aligned = prediction_scores[is_paired.view(-1) == 1]
        labels_aligned = labels[is_paired.view(-1) == 1]

        criterion = nn.CrossEntropyLoss()
        if labels_aligned.shape[0] > 0:
            masked_lm_loss = criterion(pred_scores_aligned.view(-1, self.config.vocab_size), labels_aligned.view(-1))

        else:
            masked_lm_loss = torch.tensor(0.0).to(device=device)

        #
        alignment_loss = criterion(alignment_scores, is_paired.view(-1).long())

        _, idx = alignment_scores.max(1)
        right_num = (is_paired.squeeze(1) == idx).sum().item()

        return {
            'right_match': right_num,
            'masked_lm_loss': masked_lm_loss,
            'alignment_loss': alignment_loss
        }


class NezhaFinetune(BertPreTrainedModel):
    def __init__(self, config):
        super(NezhaFinetune, self).__init__(config)
        self.bert = BertModel(config)
        self.im_to_embedding = nn.Linear(2048, 768)
        self.text_embedding = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)

        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, 13)
        self.apply(self.init_bert_weights)
    ...
    def forward(self,
                ...
                visual_token_type_id, ):
        embeds = self.text_embedding(input_ids)
        image_embeds = self.im_to_embedding(unmasked_patch_features)
        fuse_embeds = torch.cat((embeds, image_embeds), 1)

        attention_mask = torch.cat((attention_mask, visual_mask), 1)
        token_id = torch.cat((token_type_ids, visual_token_type_id + 1), 1)

        _, pooled_output = self.bert(inputs_embeds=fuse_embeds,
                                     token_type_ids=token_id,
                                     attention_mask=attention_mask,
                                     )
...
class NeZhaForJointLSTM(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.slot_classifier = nn.Linear(300 * 2, 12)
    ...
    def forward(self,
        ...
        last_encoded_layer, pooled_output = self.bert(inputs_embeds=fuse_embeds,
                                                      token_type_ids=token_id,
                                                      attention_mask=attention_mask,
                                                      )
        intent_logits = self.intent_classifier(self.dropout(pooled_output))

        slot_logits, (h_n, c_n) = self.lstm(last_encoded_layer)
        out = torch.cat((h_n[-1, :, :], h_n[-2, :, :]), dim=-1)  # 拼接正向最后一个输出和反向最后一个输出

        logits = self.slot_classifier(out)

        output = torch.cat((intent_logits, logits), 1)

        return output


class NeZhawithAttn(BertPreTrainedModel):
    def __init__(self, config):
        super(NeZhawithAttn, self).__init__(config)
        self.bert = BertModel(config)
        self.im_to_embedding = nn.Linear(2048, 768)
        self.text_embedding = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
        ...
        self.map1 = nn.Sequential(
            nn.Linear(config.hidden_size, 480),
            nn.ReLU(),
            nn.Dropout(0.5),
            nn.Linear(480, config.hidden_size)
        )

        self.map2 = nn.Linear(36, 12)
    ...
    def forward(self,
        ...
        text_enc = last_encoded_layer[:, 2:-1]
        img_enc = last_encoded_layer[:, -1]

        text_map = self.map2(text_enc.transpose(1, 2)).transpose(1, 2)
        img_map = self.map1(img_enc).reshape(img_enc.shape[0], 12, -1)

        fushion = torch.cat((text_map, img_map), 2)

        logits = self.classifier(fushion).squeeze()
...
class NezhaBase(BertPreTrainedModel):
    def __init__(self, config):
        super(NezhaBase, self).__init__(config)
        self.bert = BertModel(config)
        self.im_to_embedding = nn.Linear(2048, 768)  # 图片编码->768
        # padding_idx 按 bert-embedding 填 0
        self.text_embedding = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
    ...
    def forward(self,
        ...
        return last_encoded_layer, pooled_output


class LstmDown(nn.Module):
    def __init__(self, pretrain_path):
        super().__init__()
        # feature extraction
        self.nezhabase = NezhaBase.from_pretrained(pretrain_path)
        config = self.nezhabase.config
        # lstm
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.intent_classifier = nn.Linear(config.hidden_size, 1)
        self.lstm = nn.LSTM(
    ...
    def forward(self,
                ...
                visual_token_type_id, ):
        # feature extraction
        last_encoded_layer, pooled_output = self.nezhabase(input_ids=input_ids,
                                                           token_type_ids=token_type_ids,
                                                           attention_mask=attention_mask,
                                                           unmasked_patch_features=unmasked_patch_features,
...
class NeZhaFuse(BertPreTrainedModel):
    def __init__(self, config):
        super(NeZhaFuse, self).__init__(config)
        self.bert = BertModel(config)
        self.im_to_embedding = nn.Linear(2048, 768)
        self.text_embedding = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
    ...
    def forward(self,
        ...
        logits_1 = self.slot_classifier(out)
        ...
        logits_2 = self.classifier(fushion).squeeze()
...
if __name__ == '__main__':
    input_ids = torch.rand((2, 38)).long()
    token_type_ids = torch.zeros((2, 38), dtype=torch.long)
    attention_mask = torch.ones((2, 38), dtype=torch.float)
    unmasked_patch_features = torch.rand(2, 1, 2048)
    visual_mask = torch.ones((2, 1), dtype=torch.float)
    visual_token_type_id = torch.zeros((2, 1), dtype=torch.long)

    model = NeZhawithAttn.from_pretrained('../nezha-base-www')
    output = model(
        input_ids=input_ids,
        token_type_ids=token_type_ids,
        attention_mask=attention_mask,
